...
datapath <- "/home/carlos/Documentos/Mestrado2019/Sbseg2019/Simulacoes/GLOBAL_TABLE_FOR_R_2"
datapath_final <- "/home/carlos/Documentos/Mestrado2019/Sbseg2019/Simulacoes/GLOBAL_FINISH_TABLE_FOR_R_2"
...
# ENTIRE TABLE
f <- read.table(datapath, row.names=NULL, header=TRUE)
basepath="/home/carlos/Documentos/Mestrado2019/Sbseg2019/Simulacoes/"
		...
		for (padap in ADAP_PARAMS) {
			for (pmk in MK_PARAMS) {
				...
				theme_bw(
					...
					(legend.background = theme_rect(fill="gray98", size=.5, linetype="dotted")) +
					theme(axis.title.x = element_text(vjust=-0.1)) + # more space below Xlabel
...
ir_vs_mk_ch_int <- function() {
  ...
  ch_vs_cth_ch_mk <- function () {
	...
	theme(axis.title.x = element_text(vjust=-0.1, size = 18.5)) + # more space below Xlabel
...
amp_quadro <- function () {
	...
	# (aspect.ratio=1/1.5) +
	# theme(axis.title.x = element_text(vjust=-0.1)) +
	# # theme(axis.title.x = element_text(vjust=-0.1, size = 18.5), axis.title.y = element_text(size = 18.5, angle = 90), axis.text.x = element_text(size=18.5*0.8), axis.text.y = element_text(size=18.5*0.8)) + # more space below Xlabel
	# ggsave(file=filename, width=14.5, height=6.75)
...
uc_vs_cth <- function() {
	...
	geom_bar(width = 0.4, position = position_dodge(width=0.75), colour="black", stat="identity") +
	...
	theme_bw() +
	#theme(legend.background = theme_rect(fill="gray98", size=.5, linetype="dotted")) +
...
Xh_vs_cth <- function () {
 ...
 ch_duration_histogram <- function() {
 	q <- read.table(datapath, row.names=NULL, header=TRUE)
...
histogram_duration_quadro_filtrado <- function () {
	...
	q <- read.table(datapath_final, row.names=NULL, header=TRUE)
  ...
  f3 <- e[e$cth=="p0.5",]
	...
	theme_bw() +
	theme(axis.title.x = element_text(vjust=-0.1)) + # more space below Xlabel
	ggsave(file="carlos.pdf", width=10, height=2.8)
...
show_best_worst_scenarios <- function() {
	...
	# labs(x = "", y = "") +
	# (legend.position = "none", axis.text.y = theme_blank(), axis.ticks = theme_blank()) +
	# theme(axis.text.x = element_text(vjust=-0.5, angle=getAngle(attr))) +
	...
	ggplot(f3, aes(factor(attr), v)) +
		...
		scale_fill_manual(values=colorRampPalette(brewer.pal(9,"Greys"))(4)) +
		labs(x = "", y = "") +
		theme(legend.position = "none", axis.text.y = theme_blank(), axis.ticks = theme_blank()) +
		theme(axis.text.x = element_text(vjust=-0.5, angle=getAngle(f3$attr))) +
		facet_wrap( ~ lbl, nrow=3) +
...
nodes_id_top <- function() {
	...
	ggplot(t, aes(x,y)) + geom_point(size=3.5) 
}
